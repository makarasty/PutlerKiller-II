...
def Завантаити_звук(НазваФайлу, базова_гучність=1.0):
    """
    Создаём pygame-звук и учтём общий коэффициент громкости (дата.volume).
    Каждый раз при отрисовке в главном файле можно этот звук перенастраивать,
    если нужно динамически менять громкость.
    """
...
def Завантаити_музику(fileName):
    """
    Музыка в pygame (в отличие от Sound) воспроизводится через mixer.music.
    Учитываем общий коэффициент громкости при запуске.
    """
...
def пулі(гра, боєзапас):
    """Отрисовываем патроны (иконки) в правом верхнем углу."""
...
def статусбар(гра, статусбар_картинка, шрифт):
    """Отрисовываем статус-бар с убитыми, пропущенными, уровнем и счётом."""
    гра.blit(статусбар_картинка, (12, 12))
    гра.blit(шрифт.render(str(
        (lambda A: '0' + str(A) if A < 10 else str(A))(дата.рівень)
    ), True, white), (184, 29))
...
def зірки(гра):
    """Простая анимация пролета звезд на фоне."""
...
def меню(гра, шрифт):
    """Отрисовка надписи 'Нажми пробел...' и подсказки про настройки."""
    if дата.блік:
        txt = шрифт.render('PRESS SPACE TO START THE GAME', True, (255,255,255))
        гра.blit(txt, txt.get_rect(
            center=(дата.Гра_ширина / 2, дата.Гра_висота / 2 + 60)))
    txt2 = шрифт.render('PRESS [S] FOR SETTINGS', True, (255,255,255))
    гра.blit(txt2, txt2.get_rect(
        center=(дата.Гра_ширина / 2, дата.Гра_висота / 2 + 120)))
...
def гра_завершенна(гра, шрифт_в, шрифт_м):
    ...
    s = шрифт_в.render(f'You record {дата.рахунок}', True, (255,255,255))
    гра.blit(s, s.get_rect(
        center=(дата.Гра_ширина / 2, дата.Гра_висота / 2)))

def убивство(гра, шрифт, рахунок, x, y, _час):
    """Анимация текста 'KILL' на месте убийства."""
    t = шрифт.render('KILL', True, (255,255,255))
    гра.blit(t, t.get_rect(
        center=(x + 100, y + 100 - дата.Ворог_k_pos)))

def clear():
    """Сбросить основные игровые переменные (при выходе или рестарте)."""
    дата.убийств = 0
    дата.рахунок = 0
    дата.постріли = 0
    дата.пропущенні = 0
...
def Генерація_зірок():
    """Сгенерировать массивы звёзд случайным образом."""
    from random import randint
    дата.Масив_зiрки = [
        [randint(0, дата.Гра_ширина), randint(0, дата.Гра_висота)]
        for _ in range(дата.Максимально_зiрок)
    ]
    дата.Масив_зiрки_2 = [
        [randint(0, дата.Гра_ширина), randint(0, дата.Гра_висота)]
        for _ in range(дата.Максимально_зiрок)
...
def Прорахувати_рахунок():
    """Добавляет очки за убийство с учётом уровня."""
